...
function create_folder($path = null, $path_thumbs = null)
{
	...
	{
		mkdir($path, CoasterCms\Helpers\Admin\FileManager::createDirPermissions(), true);
	} // or even 01777 so you get the sticky bit set
	if ($path_thumbs && ! file_exists($path_thumbs))
	{
		mkdir($path_thumbs, CoasterCms\Helpers\Admin\FileManager::createDirPermissions(), true) or die("$path_thumbs cannot be found");
	} // or even 01777 so you get the sticky bit set
	umask($oldumask);
}
...
function is_really_writable($dir)
{
	...
	if (is_dir($dir))
	{
		...
		fclose($fp);
		@chmod($dir, CoasterCms\Helpers\Admin\FileManager::createDirPermissions());
		@unlink($dir);
...
function rcopy($source, $destination, $is_rec = false)
{
	if (is_dir($source))
	{
		...
		{
			mkdir($destination, CoasterCms\Helpers\Admin\FileManager::createDirPermissions(), true);
		}
